        ...
        switch (e.type)
        {
                ...
                switch (e.key.keysym.sym)
            {
                ...
                case SDLK_d: // si appuie sur D, il demande les détails
                    ...
                    {
                        if(partie_contrelamontre.get_nombre(j-1)<11)
                        {
                            img = IMG_LoadTexture(renderer,detail.get_detail(partie_contrelamontre.get_nombre(j-1)).c_str()); // charge l'image
                            SDL_QueryTexture(img, NULL, NULL, &w, &h); // recuperer la longueur et largeur
                        }
                        
                     /*   else //si jamais on est après j=10, on a un décalage de l'itérateur dans la fonction Questions
                        {
                            img = IMG_LoadTexture(renderer,detail.get_detail(partie_contrelamontre.get_nombre(j+1)).c_str()); // charge l'image
                            SDL_QueryTexture(img, NULL, NULL, &w, &h); // recuperer la longueur et largeur
                            std::cout << partie_contrelamontre.get_nombre(j) << std::endl;
