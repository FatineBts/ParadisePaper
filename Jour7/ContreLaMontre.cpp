...
{
    _page.push_back("/Users/fatine/Desktop/Cpp/Cpp/IMG/contrelamontre.jpg");
...
void ContreLaMontre::choix_question(Questions question, Reponses reponse)
{
    ...
    int tmp;
    srand(time(NULL)); //pour avoir des nombres aléatoires dépendant du temps qui changent d'une partie à l'autre  
    for(int i=0; i<5; i++)
    {
        tmp=rand()%31;
   ...
   ///Partie 2: on récupère les 5 images correspondantes
    for(int i=0; i<5; i++)
    {
        ...
        _PATH_reponses[i]=reponse.operator()(_nombre[i]); //on prend les réponses correspondantes aux questions
