...
Score::Score(){_score=0; _essai=1; _a_vu_reponse=false;} //on met le score à 0
...
void Score::resultat(int numero_question, int mode, std::string touche) // en fonction de si la personne donne la bonne réponse ou pas
{
    ...
    std::cout << "reponse_score:" << reponse_faite << std::endl;
     std::cout << "verdict:" << verdict << std::endl;
    
    if(reponse_faite==touche && verdict==false) //si il appuie sur le bon résultat et qu'il n'a pas vu la réponse
    {
        _score= _score +1; // si il appuie sur la bonne touche on augmente son score)
        std::cout << "_score_boucle=" << _score << std::endl;
        //_essai=false; //le score ne sera augmenté que si elle a raison à la première tentative
    }

    else
        _essai=2; // on bloque les possibles erreurs de score
    
    std::cout << "i=" << numero_question << "score_orboucle="<<get_score() << std::endl;
